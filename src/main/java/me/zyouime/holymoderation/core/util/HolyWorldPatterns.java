package me.zyouime.holymoderation.core.util;
...
public final class HolyWorldPatterns {
    ...
    public static final List<String> IGNORED_PREFIXES = List.of("[ALL] ʟ", "[Тихий] ❖", "SC |", "HW >", " ▬▬▬", "▬▬▬", "[PMS]:", "◀", "[HM]", "[HAC]", "[я");
    ...
    public static final String LOBBY_NO_COMMAND = "Лобби ▶ Команды не существует";
    ...
    private static final char CLAN_OPEN = '«';
    private static final char CLAN_CLOSE = '»';
    ...
    public static boolean isLobbyNoCommand(String text) {
        return text.equals(LOBBY_NO_COMMAND);
    }

    public static String chatSender(String plain) {
        String body = plain.startsWith(ALL_PREFIX) ? plain.substring(ALL_PREFIX.length()) : plain;
        if (body.isEmpty() || !isChatMarker(body.charAt(0))) {
            return null;
        }
        String rest = body.substring(1).trim();
        if (!rest.isEmpty() && rest.charAt(0) == CHANNEL_SEPARATOR) {
            rest = rest.substring(1).trim();
        }
        String[] tokens = rest.split(" ");
        int index = skipClanTag(tokens);
        if (index >= tokens.length) {
            return null;
        }
        return trimTrailingColon(tokens[index]);
    }

    private static boolean isChatMarker(char first) {
        for (char marker : CHAT_MARKERS) {
            if (first == marker) {
                return true;
            }
        }
    ...
    private static int skipClanTag(String[] tokens) {
        if (tokens.length == 0 || tokens[0].isEmpty() || tokens[0].charAt(0) != CLAN_OPEN) {
            return 0;
        }
        for (int index = 0; index < tokens.length; index++) {
            if (tokens[index].endsWith(String.valueOf(CLAN_CLOSE))) {
                return index + 1;
            }
        }
    ...
    private static String trimTrailingColon(String nickname) {
    ...
    public static String parseFindStatus(String findLine) {
        if (findLine.equals(FIND_OFFLINE)) {
            return "offline";
        }
    ...
    public static PunishmentKind classifyPunishment(String line) {
        if (!isHistoryPlayerLine(line)) {
            return null;
        }
        if (line.contains(HISTORY_BAN_MARK)) {
            return PunishmentKind.BAN;
        }
        if (line.contains(HISTORY_KICK_MARK)) {
            return PunishmentKind.KICK;
        }
        if (line.contains(HISTORY_MUTE_MARK)) {
            return PunishmentKind.MUTE;
        }
        return null;
    }

    public static String extractHistoryReason(String line) {
        if (!line.startsWith(HISTORY_REASON_PREFIX)) {
