...
public record Exec(LoggerService logger, ModuleManager manager, NotificationsService notificationsService, String commandName) {

    public Command<FabricClientCommandSource> of(Action action) {
        return context -> {
            if (!manager.isEnabled()) {
                notificationsService.warning("Мод выключен, команды не работают. Войдите на HolyWorld");
                return 0;
            }
            try {
                action.run(context);
